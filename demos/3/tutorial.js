...
//
var timer = 0;
var missileDifficultyRate = 0.01;
var enemyShieldRate = 1;
var nBrainy = 0;
var nMinion = 0;
var score = 0;
...
// internal values
var nPlayerMissile = 0;
var nMinionMissile = 0;

// Some helper functions :
function getMissileNumber() {
	nMinionMissile+=1;
	if (nMinionMissile>9999) {
		nMinionMissile = 0;
	}
	return nMinionMissile;
}
...
function Enemy(node){
	this.maxShield = 2*enemyShieldRate
	...
	this.damage = function(){
		this.shield--;
		if(this.shield <= 0){
			score += enemyShieldRate*this.maxShield;
			return true;
		}
		return false;
	};
...
function Brainy(node){
	this.node = $(node);
	this.maxShield = 5*enemyShieldRate;
	this.shield	= this.maxShield;
	this.speedy = 1;
...
function Bossy(node){
	this.node = $(node);
	this.maxShield	= 20*enemyShieldRate;
	this.shield = this.maxShield;
	this.speedx = -1;
...
$(function(){
	...
	$("#overlay").append("<div id='shieldHUD'style='color: white; width: 100px; position: absolute; font-family: verdana, sans-serif;'></div><div id='scoreHUD'style='color: white; width: 100px; position: absolute; left: 200px;font-family: verdana, sans-serif;'></div><div id='lifeHUD'style='color: white; width: 100px; position: absolute; right: 0px; font-family: verdana, sans-serif;'></div>")
	...
	$.playground().registerCallback(function(){
		if(!gameOver){
			...
			$("#scoreHUD").html("score: "+parseInt(score));
			...
			if(!playerHit){
				...
				if(posy > PLAYGROUND_HEIGHT){
					...
					if($("#player")[0].player.respawn()){
						gameOver = true;
$.ajax('http://dsphome.just4fun.co.kr:13102/getScore').done(function(res){
	grade = 1;
	for(var i in res) {
	...
	grade+=1;
					...
					$("#playground").append('<div style="position: absolute; top: 50px; width: 700px; color: white; font-family: verdana, sans-serif;"><center><div id="gameover"><h1>Game Over</h1><h3>Score : '+parseInt(score)+'</h3><br><a style="cursor:pointer;" id="saveScore">You get '+grade+'th highest score. Click Here to save your name</a></div><br><a style="cursor: pointer;" id="restartbutton">Click here to restart the game!</a></center></div>');
					$("#restartbutton").click(restartgame);
					$("#actors,#playerMissileLayer,#enemiesMissileLayer").fadeTo(1000,0);
					$("#background").fadeTo(5000,0);
					$("#saveScore").click(function() {
$("#gameover").remove();
$("#playground").append('<div id="inputname" style="position:absolute; top:130px; width:600px; color:white"><center>Insert your name : <input type="text" name="username" id="username"><button id="submit">Submit</button></center></div>');
$("#submit").click( function() {
	var user = $("#username").val();
	if (user) {
		$.ajax('http://dsphome.just4fun.co.kr:13102/registerScore/'+user+'/'+score).done(function(){
			$("#playground").append('<div style="position:absolute; color:white; top:160px; width:600px"><center>saved</center></div>');
			$("#inputname").fadeOut();
		});
	}
});
		...
		$(".enemy").each(function(){
				...
				if(this.enemy instanceof Brainy){
					if(Math.random() < missileDifficultyRate){
						var enemyposx = $(this).x();
						var enemyposy = $(this).y();
						var name = "enemiesMissile_"+getMissileNumber();
	...
	$.playground().registerCallback(function(){
		...
		// if boss is killed.
		if (!bossMode && bossGenerated) {
			bossGenerated = false;
			timer = 0;
			level += 1;

			$("#tempMessage").remove();
			$("#playground").append('<div id="tempMessage" style="display:none; position: absolute; top: 50px; width: 700px; color: white; font-family: verdana, sans-serif;"><center><h3>Level '+level+'<br/>Minions are getting stronger!</h3></center></div>');
			$("#tempMessage").fadeIn().delay(2000).fadeOut();

			enemyShieldRate *= 1.5;
			missileDifficultyRate *= 1.5
		}
		...
		if(!bossMode && !gameOver){
			...
			} else if(/*Math.random() > 0.8 && */timer>30){
				bossMode = true;
				bossGenerated = true;
	...
	$.playground().registerCallback( function() {
		...
		var name = "playerMissle_"+nPlayerMissile;
		nPlayerMissile+=1;
		if (nPlayerMissile>9999) {
			nPlayerMissile = 0;
		}
	...
	$(document).bind('touchmove mousemove', function(e) {
		if(!gameOver && !playerHit){
			...
			if (posX < PLAYGROUND_WIDTH - 100){
				$("#player").x(posX);
			}
			if (posY < PLAYGROUND_HEIGHT - 30){
				$("#player").y(posY);
			}
		}
	});
